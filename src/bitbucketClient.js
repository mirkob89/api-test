...
export class BitbucketClient {
  constructor({ baseUrl, workspace, repoSlug, username, appPassword, dryRun = false }) {
    this.baseUrl = baseUrl?.replace(/\/$/, "") || "https://api.bitbucket.org/2.0";
  ...
  async createCommit({ branch, message = "Initial commit", filePath = "README.md", content = "Created by automation." }) {
    ...
    if (this.username && this.appPassword) {
      const authToken = Buffer.from(`${this.username}:${this.appPassword}`).toString("base64");
      headers["Authorization"] = `Basic ${authToken}`;
    }

    const { data } = await axios.post(`${this.baseUrl}${url}`, form, { headers });
    return data;
  }
}
